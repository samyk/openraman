...
EventMonitorApp* g_pMainApplication = nullptr;
...
WNDCLASS makeWndClass(HINSTANCE hInstance)
{
	...
	hWndClass.hbrBackground = (HBRUSH)CreateSolidBrush(RGB(240,240,240));
	hWndClass.lpszMenuName = NULL;
	hWndClass.lpszClassName = TEXT(EVEMON_CLASSNAME);
...
{
	_debug("Starting EventMonitoring");

	// check if program is already opened
	HWND hPrevWnd = FindWindow(TEXT(EVEMON_CLASSNAME), NULL);
	...
	try
	{
		...
		// create application
		g_pMainApplication = new EventMonitorApp(hInstance);
